...
function create() {
  ...
  belowLayer = map.createLayer("Ground", tileset, 0, 0);
  worldLayer = map.createLayer("Fences", tileset, 0, 0);

  audio = []
  audio["move"] = this.sound.add('audio_move');
  audio["turn"] = this.sound.add('audio_turn');
  audio["stuck"] = this.sound.add('audio_stuck');

  player = this.physics.add.sprite(95, 155, "cat");
  player.setFrame(2);
  player.direction = 2;
...
function turn(step) {
  ...
  new_direction = player.direction + step
  if (new_direction > 3) {
    new_direction = 0;
...
function move() {
  ...
  tile = worldLayer.getTileAtWorldXY(new_x, new_y, true);
  if(tile.index === -1) {
    audio["move"].play();
    scene.tweens.add({
      targets: player,
...
function stepCode() {
  ...
  if (interpreter.step()) {
    ...
    window.setTimeout(stepCode, delay);
  }
}
...
document.addEventListener("DOMContentLoaded", function(event) {
  ...
  document.getElementById("run_code").addEventListener("click", runCode);
