class PlayerToolbar extends Phaser.GameObjects.Container {
  ...
  constructor(scene, x, y, texture, name) {
    ...
    this.coinText = this.scene.add.text(145, 48, '0', {fontFamily: '"Press Start 2P"', fontSize: '12px', color: '#ffffff', fill: '#000'}).setOrigin(0.5, 0.5);
    this.ruby = this.scene.add.image(175, 46, 'textures', 'Ruby').setScale(0.7).setOrigin(0.5, 0.5);
    this.rubyText = this.scene.add.text(205, 48, '0', {fontFamily: '"Press Start 2P"', fontSize: '12px', color: '#ffffff', fill: '#000'}).setOrigin(0.5, 0.5);
    this.crystal = this.scene.add.image(230, 48, 'textures', 'Crystal').setScale(0.6).setOrigin(0.5, 0.5);
    this.crystalText = this.scene.add.text(255, 48, '0', {fontFamily: '"Press Start 2P"', fontSize: '12px', color: '#ffffff', fill: '#000'}).setOrigin(0.5, 0.5);
